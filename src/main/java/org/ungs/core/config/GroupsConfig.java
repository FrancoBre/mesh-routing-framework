...
public record GroupsConfig(
    Map<String, List<Integer>> groups // name -> node ids
) {

    public static GroupsConfig fromLoader(SimulationConfigLoader l) {
        List<String> names = (l.groups() == null) ? List.of() : l.groups().stream()
            .map(String::trim)
            .filter(s -> !s.isEmpty())
            .toList();
        ...
        for (String name : names) {
            ...
            if (raw == null || raw.isBlank()) {
                throw new IllegalArgumentException(key + " must be set when group '" + name + "' is listed in groups");
            }
